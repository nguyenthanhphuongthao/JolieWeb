		...
		<form class="infoBill">
			<div class="row">
				...
				<div class="col-md-4">
					...
					<c:choose>
						<c:when test="${billinfo.configByPaymentMethod.id == 24}">
							<h5>Thanh toán khi nhận hàng</h5>
						</c:when>
						<c:when test="${billinfo.configByPaymentMethod.id == 25}">
					...
					<c:choose>
						<c:when test="${billinfo.configByShippingMethod.id == 27}">
							<h5>Giao trong 48h</h5>
						</c:when>
						<c:when test="${billinfo.configByShippingMethod.id == 26}">
							<h5>Giao hỏa tốc</h5>
						</c:when>
					</c:choose>
				</div>
